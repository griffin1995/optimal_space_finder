...
def run_optimizer_for_radius(optimizer, radius):
    ...
    visualization = optimizer.visualize_map_with_optimal(
        (optimal_x, optimal_y), view_radius=radius
    )
    plt.title(
        f"Spawn Map - Radius {radius} - Optimal Position: ({optimal_x}, {optimal_y})"
    )
    ...
    plt.title(
        f"Coverage Heatmap - Radius {radius} - Visible Spawns: {coverage} ({coverage_percentage:.1f}%)"
    )
...
def main():
    ...
    for radius, opt_x, opt_y, coverage, percentage in results:
        print(
            f"{radius:<10}({opt_x:<3}, {opt_y:<3}){'':>10}{coverage:<20}{percentage:<15.2f}"
        )
    ...
    with open("radius_analysis_summary.txt", "w") as f:
        ...
        f.write("=" * 80 + "\n")
        f.write(
            f"{'Radius':<10}{'Position':<20}{'Visible Spawns':<20}{'Coverage %':<15}\n"
        )
        f.write("-" * 80 + "\n")
    ...
    print(
        "Comparison charts saved to radius_comparison_chart.png and visible_spawns_chart.png"
    )
